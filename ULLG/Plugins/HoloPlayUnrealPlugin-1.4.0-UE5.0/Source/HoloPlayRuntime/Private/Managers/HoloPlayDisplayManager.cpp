...
struct FDisplayManagerHelper
{
	...
	static int GetModel(int ScreenIndex)
	{
		TSharedPtr<FJsonObject> Device = GetDeviceForScreen(ScreenIndex);
		if (Device.IsValid())
		{
			FString HardwareVersion = Device->GetStringField("hardwareVersion");
            if ( HardwareVersion == "portrait" )
            {
                return 0;
            }
			if (HardwareVersion == "standard")
			{
				return 1;
			}
			if (HardwareVersion == "large")
			{
				return 2;
			}

			UE_LOG(HoloPlayLogManagers, Error, TEXT("Screen %d hardware Version bad: %s"), ScreenIndex, *HardwareVersion);
		}
		return 0;
	}
    ...
    {
        TSharedPtr<FJsonObject> Device = GetDeviceForScreen( ScreenIndex );
		EHoloPlayDeviceType result = EHoloPlayDeviceType::Portrait;
        if ( Device.IsValid() )
        {
			int iScreenWidth  = GetScreenWidth(ScreenIndex);
			int iScreenHeight = GetScreenHeight(ScreenIndex);

			
			if ( iScreenWidth == 1536 && iScreenHeight == 2048 )
			{
				result = EHoloPlayDeviceType::Portrait;
			}

            if ( iScreenWidth == 3840 && iScreenHeight == 2160 )
            {
                result = EHoloPlayDeviceType::FourK;
            }

            if ( iScreenWidth == 7680 && iScreenHeight == 4320 )
            {
                result = EHoloPlayDeviceType::EightK;
            }

            if ( iScreenWidth == 2560 && iScreenHeight == 1600 )
            {
                result = EHoloPlayDeviceType::EightPointNineInchLegacy;
            }
...
{
	Type = FDisplayManagerHelper::GetDeviceType(i);
	Model = FDisplayManagerHelper::GetModel(i);
...
{
	Type = EHoloPlayDeviceType::Portrait;
    Model = 0;
    ScreenWidth = 1536;
    ScreenHeight = 2048;
    DPI = 324.0f;
...
bool FHoloPlayDisplayManager::SetHoloPlayDisplaySettings()
{
	...
	if (bIsServiceRunning)
	{
		...
		// Use default  off line calibration when no device is detected
